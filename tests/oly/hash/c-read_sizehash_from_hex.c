/* read_sizehash_from_hex test License GPL2+ {{{
...
main( int argc, char **argv ){
    ...
    if (OLY_OKAY != read_sizehash_from_hex (read_me, read, oly))
    {
        printf("Problem with reading the sizehash.\n%s",read);
        exit(EXIT_FAILURE);
    }
    ...
    if (strcmp_val!=0) {
        ...
        print_hex_from_sizehash(read, oly);
        printf("\nValue hashed:\n");
        print_hex_from_sizehash(input, oly);
        printf("\nStrcmp_val: %i\n", strcmp_val);
        exit(EXIT_FAILURE);
    }
    printf("Success!\n");
