/* init_locale.c - returns the best match locale for the user. License GPL2+ {{{
...
OlyStatus init_locale (char *locale[], OlyStatus *status)
{
    ...
    char            *r, result[OLY_SMALL_BUFFER];
    *status         = OLY_OKAY;
    r=result;
    
    output_size = uloc_getBaseName(*locale, r, OLY_SMALL_BUFFER, &u_status);
